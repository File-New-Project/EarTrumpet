...
public class AppPopup : Popup
{
    ...
    {
        var root = ((FrameworkElement)Child) ?? throw new ArgumentException("Child");
        var container = (FrameworkElement)PlacementTarget ?? throw new ArgumentException("PlacementTarget");
        var relativeTo = Window.GetWindow(this);

        var offsetFromWindow = container.TranslatePoint(new Point(0, 0), relativeTo);
